...
Includes a voice validator that evaluates each draft against 8 checks
derived from Dana's real writing patterns. If more than 1 check fails,
the note is regenerated with specific feedback — up to 3 attempts.
Notes still failing after 3 attempts are saved with _NEEDS-REVIEW.
"""
...
_SYSTEM = """\
You write Substack Notes for Skitsa in Dana's exact voice.
...
def generate_note(insight: dict, field_config: dict,
...
def validate_note(note: str, insight: dict) -> tuple[bool, list[str], str, dict]:
    """
    ...
    prompt = (
        ...
        f"INSIGHT TITLE: {insight.get('title', '')}\n\n"
        f"DRAFT:\n{note}"
    )
...
def generate_and_validate_note(
    ...
    for attempt in range(1, max_attempts + 1):
        note = generate_note(insight, field_config, feedback=feedback)
        passed, failed_checks, feedback, checks = validate_note(note, insight)
...
def save_note(insight: dict, note: str, field_config: dict,
    ...
    filename = f"{today}_skitsa_{slug}{review_flag}.md"
